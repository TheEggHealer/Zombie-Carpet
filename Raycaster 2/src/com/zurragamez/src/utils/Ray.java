...
public class Ray {
	...
	public Wall cast(int scanX) {
		...
		while(hit == 0) {
			...
			if(World.map[cellX][cellY] > 0) hit = 1;
		}
		...
		return new Wall(scanX, Main.HEIGHT / 2 - height / 2, height, World.map[cellX][cellY], (float)perpWallDist, texX, floorXWall, floorYWall, (float)dx, (float)dy, (float)(dy + perpWallDist * dirY * 0.99f), (float)(dx + perpWallDist * dirX * 0.99f));
	}
